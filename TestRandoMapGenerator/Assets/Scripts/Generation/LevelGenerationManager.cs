...
public class LevelGenerationManager : MonoBehaviour
{
    ...
    [field: SerializeField] private List<Room> _rooms = new List<Room>();
    [field: SerializeField] private List<Room> _inBetweenrooms = new List<Room>();
    [field: SerializeField] private List<RoomObject> _roomObjects = new List<RoomObject>();
    ...
    private bool CheckBuildingBlocks()
    {
        ...
        if (InBetweenRooms.Length == 0) { Debug.LogError("NotEnoughInbetweenRooms"); return false; }
    ...
    private void SpawnNextRoom()
    {
        ...
        if (_rooms.Count -2 < steps) { 
            SpawnRoom(Difficulty.easy);
            SpawnRoom(Difficulty.inBetween);
        } //-2 WEIL vorher der startraum und ein easy room reingeladen wurde
        else if (_rooms.Count -2 >= steps && _rooms.Count -2 < steps * 2) { 
            SpawnRoom(Difficulty.medium);
            SpawnRoom(Difficulty.inBetween);
        }
        else if (_rooms.Count -2 >= steps * 2 ) { 
    ...
    private void SpawnRoom(Difficulty difficulty)
    {
        ...
        { 
            ...
            case Difficulty.easy:
                ...
                break;
            case Difficulty.inBetween:
                ranomValue = UnityEngine.Random.Range(0, InBetweenRooms.Length);
                spawnedRoom = Instantiate(InBetweenRooms[ranomValue].RoomObject, CurrentRoom.ExitList[CalculateNextExit()].transform);
                _inBetweenrooms.Add(InBetweenRooms[ranomValue]);
                lastRoomDif = Difficulty.inBetween;
                break;
            default:
...
{
    easy, medium, hard, inBetween, notDefined
}
